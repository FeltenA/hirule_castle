...
boss_selection(){
    ...
    #Save all the charactetistics of the bosses
    #0=name, 1=max hp, 2=max mp, 3=str, 4=int, 5=res, 6=spd, 7=luck
    #8=dynamic hp, 9=dynamic mp, 10=race, 11=class
...
combat_display(){
    ...
    then
	echo -e "\n---Options------------------\n1.Attack   2.Heal   3.Protect   4.Escape"
    fi
	...
	if [[ $ginput != "protect" ]] && [[ $ginput != "Protect" ]] && [[ $ginput != 3 ]] && [[ $ginput != "escape" ]] && [[ $ginput != "Escape" ]] && [[ $ginput != 4 ]]
	then
...
combat_turn(){
    ...
    #If player doesn't use protect or escape
    if [[ ${enemie[8]} -gt 0 ]] && [[ $input != "protect" ]] && [[ $input != "Protect" ]] && [[ $input != 3 ]] && [[ $input != "escape" ]] && [[ $input != "Escape" ]] && [[ $input != 4 ]]
    then
	player[8]=$((${player[8]}-${enemie[3]}))
    fi
    fight=$(($fight+1))
    better_combat_options $input
    combat_display
}
